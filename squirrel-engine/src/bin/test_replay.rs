...
async fn main() -> Result<(), sqlx::Error> {
    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://postgres:postgres@localhost/squirrel_db".to_string());
    let pool = PgPoolOptions::new().connect(&database_url).await?;

    let row = sqlx::query("SELECT id, replay_events FROM matches ORDER BY id DESC LIMIT 1")
        .fetch_one(&pool)
        .await?;
    ...
    match res2 {
        Ok(v) => println!(
            "Match ID {}: Decoded sqlx::types::Json len: {}",
            id,
            v.0.len()
        ),
    ...
    match res3 {
        Ok(v) => println!(
            "Match ID {}: Decoded Value is_array: {}, is_null: {}",
            id,
            v.is_array(),
            v.is_null()
        ),
