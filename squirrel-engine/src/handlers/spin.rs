...
pub async fn get_spin_info(
    ...
    let spin_row =
        sqlx::query("SELECT last_free_spin_at FROM user_lucky_spins WHERE telegram_id = $1")
...
pub async fn draw_spin(
    ...
    if payload.use_free {
        if !can_spin_free {
            return Err((
                ...
                "Free spin not yet available".to_string(),
            ));
        }
    } else {
        // deduct 500 nuts
        ...
        sqlx::query("UPDATE users SET free_coins = free_coins - 500 WHERE telegram_id = $1")
            ...
            .map_err(|e| {
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    format!("Failed to deduct nuts: {}", e),
                )
            })?;
    }
    ...
    .map_err(|_| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            "Failed to fetch rewards".to_string(),
        )
    })?;
    ...
    let dist = WeightedIndex::new(&weights).map_err(|_| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            "Weight calculation error".to_string(),
        )
    })?;
    let mut rng = OsRng;
    ...
    if r_type == "nuts" {
        sqlx::query("UPDATE users SET free_coins = free_coins + $1 WHERE telegram_id = $2")
            ...
            .map_err(|e| {
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    format!("Failed to grant nuts: {}", e),
                )
            })?;
    } else if r_type == "cosmetic" {
        if let Some(i_id) = item_id {
            if i_id.contains("chest") {
                let chest_type = if i_id.contains("rare") {
                    "rare"
                } else if i_id.contains("epic") {
                    "epic"
                } else if i_id.contains("legendary") {
                    "legendary"
                } else {
                    "common"
                };
